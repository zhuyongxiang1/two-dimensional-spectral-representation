...
from datasets import get_dataset, HyperX, open_file, DATASETS_CONFIG, HyperX2
from models import get_model, train, test, save_model, train2, test2
...
parser.add_argument(
    "--dataset", type=str, default="PaviaU", choices=dataset_names, help="Dataset to use."
)
parser.add_argument(
    "--model",
    type=str,
...
if palette is None:
    # Generate color palette {1:{192,192,192}} {2:{0,255,1}}  {3:{0,255,255}}
    # {4:{0,132,0}} {5:{255,0,254}} {6:{165,82,40}} {7:{129,0,127}} {8:{254,0,0}} {9:{255,255,0}}
...
# palette = {
#     0: (0, 0, 0),
#     1: (192, 192, 192), 2: (0, 255, 1), 3: (0, 255, 255),
#     4: (0, 132, 0), 5: (255, 0, 254),6: (165, 82, 40),
#     7: (129, 0, 127), 8: (254, 0, 0), 9: (255, 255, 0)
# }
# invert_palette = {v: k for k, v in palette.items()}
...
for run in range(N_RUNS):
    ...
    if MODEL == "SVM_grid":
        ...
        X_train, y_train = build_dataset(img, train_gt, ignored_labels=IGNORED_LABELS)
      #  clf = sklearn.ensemble.RandomForestClassifier(n_estimators=200, min_samples_split=3, max_features=10,max_depth=20)
        clf = sklearn.ensemble.RandomForestClassifier(n_estimators=200, min_samples_split=2, max_features=40,max_depth=60)
        clf.fit(X_train, y_train)
        #  save_model(clf, MODEL, DATASET)
        prediction = clf.predict(img.reshape(-1, N_BANDS))
        prediction = prediction.reshape(img.shape[:2])
        ...
        # train_gt, val_gt = sample_gt(train_gt, 0.5, mode="random")
        train_dataset = HyperX2(img, train_gt, **hyperparams)
        # val_dataset = HyperX2(img, val_gt, **hyperparams)
        #     val_gt = HyperX2(img, val_gt, **hyperparams)
        test_dataset = HyperX2(img, test_gt, **hyperparams)
        train_loader = data.DataLoader(train_dataset, batch_size=hyperparams["batch_size"], shuffle=True, num_workers=4)
        test_loader = data.DataLoader(test_dataset, batch_size=hyperparams["batch_size"], num_workers=4)
        print(hyperparams)
        print("Network :")
        # with torch.no_grad():
        ...
        try:
            train2(model, optimizer, loss, train_loader, hyperparams["epoch"], scheduler=hyperparams["scheduler"],
                   device=hyperparams["device"], supervision=hyperparams["supervision"], display=viz, )
        except KeyboardInterrupt:
            pass
        prediction = test2(model, img, test_loader, hyperparams)
    elif MODEL == "AlexNet" or MODEL == "convnext" or MODEL == "densenet" or MODEL == "efficientnet" or MODEL == "regnet" or MODEL == "vgg" or MODEL == "vit" or MODEL == "wide_resnet":
        model, optimizer, loss, hyperparams = get_model(MODEL, **hyperparams)
        # train_gt, val_gt = sample_gt(train_gt, 0.5, mode="random")
        train_dataset = HyperX2(img, train_gt, **hyperparams)
        # val_dataset = HyperX2(img, val_gt, **hyperparams)
        ...
        #  val_loader = data.DataLoader(val_dataset,# pin_memory=hyperparams['device'],batch_size=hyperparams["batch_size"],)
        print(hyperparams)
        # print("Network :")
        # with torch.no_grad():
        ...
        try:
            train(model, optimizer, loss, train_loader, hyperparams["epoch"], scheduler=hyperparams["scheduler"],
    ...
    show_results(run_results, viz, label_values=LABEL_VALUES, dataset=DATASET,run=run, model=MODEL,tr=SAMPLE_PERCENTAGE)
 #   print(time_sum)
    all_time_sum += time_sum


if N_RUNS > 1:
    show_results(results, viz, label_values=LABEL_VALUES, agregated=True, dataset=DATASET,run=run, model=MODEL,tr=SAMPLE_PERCENTAGE)
