...
def get_model(name, **kwargs):
    ...
    #     model.fc = nn.Sequential(nn.Linear(num_ftrs, n_classes), nn.LogSoftmax(dim=1))
    #     lr = kwargs.setdefault("learning_rate", 0.03)
    #     optimizer = optim.SGD(model.parameters(), lr=lr)  # mymodule.parameters()
    ...
    #     criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        center_pixel = True
        patch_size = kwargs.setdefault("patch_size", 1)
        model = models.AlexNet(dropout=0.7)
        model.classifier.add_module("add_linear", nn.Linear(1000, n_classes))
      #  model.fc = nn.Sequential(nn.Linear(num_ftrs, n_classes), nn.LogSoftmax(dim=1))
        lr = kwargs.setdefault("learning_rate", 0.03)
        optimizer = optim.SGD(model.parameters(), lr=lr)  # mymodule.parameters()
        kwargs.setdefault("batch_size", 100)
        ...
        #     # 也就是采用该模型已经训练好的原始参数。
        # 只需要训练我们自己定义的Linear层
        model.classifier = nn.Sequential(nn.Linear(num_ftrs, n_classes), nn.LogSoftmax(dim=1))
        #  model.classifier.add_module("add_linear", nn.Linear(1000, n_classes))
        lr = kwargs.setdefault("learning_rate", 0.03)
        optimizer = optim.SGD(model.parameters(), lr=lr)  # mymodule.parameters()
        kwargs.setdefault("batch_size", 100)
    ...
    #     criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        num_ftrs = model.fc.in_features
        model.fc = nn.Sequential(nn.Linear(num_ftrs, n_classes), nn.LogSoftmax(dim=1))
        # model.fc.add_module("add_linear", nn.Linear(1000, n_classes))
        lr = kwargs.setdefault("learning_rate", 0.03)
        optimizer = optim.SGD(model.parameters(), lr=lr)  # mymodule.parameters()
        kwargs.setdefault("batch_size", 100)
        kwargs.setdefault("epoch", 300)
        ...
        model.heads.add_module("add_linear", nn.Linear(1000, n_classes))
        lr = kwargs.setdefault("learning_rate", 0.03)
        optimizer = optim.SGD(model.parameters(), lr=lr)  # mymodule.parameters()
        kwargs.setdefault("batch_size", 100)
        kwargs.setdefault("epoch", 300)
    ...
    epoch = kwargs.setdefault("epoch", 300)
    # kwargs.setdefault(
    #     "scheduler",
    ...
    #     ),
    # )
   # kwargs.setdefault("scheduler", optim.lr_scheduler.StepLR(optimizer, step_size=40, gamma=0.5), )
    kwargs.setdefault("scheduler", optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.9), )
    ...
    def __init__(self, band, classes):
        ...
        self.conv12 = nn.Sequential(
          #  nn.Conv2d(in_channels=band, out_channels=128, kernel_size=(3, 3)),
            nn.Conv2d(in_channels=band, out_channels=128, kernel_size=(3, 3), padding="same"),
            nn.MaxPool2d(kernel_size=(5, 5))
            # nn.MaxPool2d(kernel_size=(3, 3))
        )
      #  self.conv13 = nn.Conv2d(in_channels=band, out_channels=128, kernel_size=(5, 5),)
        self.conv13 = nn.Sequential(
            nn.Conv2d(in_channels=band, out_channels=128, kernel_size=(5, 5),padding="same"),
            nn.MaxPool2d(kernel_size=(5, 5))
        )
...
def train2(
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        # Run the training loop for one epoch
        for batch_idx, (data, target, _, _) in tqdm(
                enumerate(data_loader), total=len(data_loader)
        ):
            # Load the data into the GPU if required
            data, target = data.to(device), target.to(device)
        ...
        avg_loss /= len(data_loader)
        if val_loader is not None:
            val_acc = val2(net, val_loader, device=device, supervision=supervision)
            val_accuracies.append(val_acc)
            metric = -val_acc
        else:
            metric = avg_loss
...
def test2(net, img, test_loader, hyperparams):
    """
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            img = img.to(device)
            output = net(img)
            output2 = output.argmax(1)
            for i in range(len(x)):
                x_img = x[i].int()
                y_img = y[i].int()
                lable_img = output2[i].int()
                #  predict.append=output
                probs[x_img, y_img] = lable_img
        # with torch.no_grad():
        #     if patch_size == 1:
...
def val2(net, data_loader, device="cpu", supervision="full"):
