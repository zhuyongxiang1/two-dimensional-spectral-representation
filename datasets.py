    ...
    def __getitem__(self, i):
      ...
      #  data = np.asarray(np.copy(data).transpose((2, 0, 1)), dtype="float32")
        ...
        if self.patch_size > 1:
            # Make 4D data ((Batch x) Planes x Channels x Width x Height)
            data = data.unsqueeze(0)
        path ="./Datasets_2d/PaviaU/{}_{}_{}.png".format(x,y,int(label))
        red_img = transforms.Compose([  # 常用的数据变换器
            lambda img: Image.open(path).convert('RGB'),  # string path= > image data
          #  if model=vit delete #
          #   transforms.Resize(
          #   (int(224 * 1.25), int(224 * 1.25))),
          #   transforms.RandomRotation(15),
          #   transforms.CenterCrop(224),  # 防止旋转后边界出现黑框部分
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                 std=[0.229, 0.224, 0.225])
        ])
        img_data = red_img(path)
       # return data, label
        return img_data, label,x,y
