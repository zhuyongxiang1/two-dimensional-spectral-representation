...
def show_results(results, vis, label_values=None, agregated=False,dataset=str,run=int,model=str,tr=int,time=float):
    text = ""
 ...
 #   text += "time: {:.04f}\n".format(time)
    ...
    with open(folder_data, 'w') as x_file:
        ...
        x_file.write('\n')
        if agregated:
            x_file.write("AA:{:.04f} +- {:.04f}\n".format(average_accuracy,average_accuracy_std))
        else:
            x_file.write("AA:{:.04f}".format(average_accuracy))
...
def sample_gt(gt, train_size, mode='random'):
    ...
    selected_indices_training = []
    selected_indices_testing = []
    selected_indices = []
    if mode == 'random':
       if train_size > 1:
           for cls in range(1,20):
                cls_indices = np.where(y == cls)
                selected_indices_training.append(cls_indices[0][:train_size])
                selected_indices_testing.append(cls_indices[0][train_size:])
           for ll in range(len(list(selected_indices_training))):
               indices_training=selected_indices_training[ll]
               indices_testing=selected_indices_testing[ll]
               # train_indices = [list(t) for t in zip(*indices_training)]
               # test_indices = [list(t2) for t2 in zip(*indices_testing)]
               #  X[indices_training]
               for ii in range(len(indices_training)):
                   x_y=X[indices_training[ii]]
                   train_gt[x_y] = gt[x_y]
               for jj in range(len(indices_testing)):
                   x_y_2 = X[indices_testing[jj]]
                   test_gt[x_y_2] = gt[x_y_2]
        #   indices1 = np.nonzero(train_gt)
        #   indices2 = np.nonzero(test_gt)
          # train_gt[tuple(train_indices)] = gt[tuple(train_indices)]
          # test_gt[tuple(test_indices)] = gt[tuple(test_indices)]
       else:
           train_indices, test_indices = sklearn.model_selection.train_test_split(X, train_size=train_size, stratify=y,random_state=1000)
         #  train_indices, test_indices = sklearn.model_selection.train_test_split(X, train_size=train_size, stratify=y)
          # train_indices, test_indices = sklearn.model_selection.train_test_split(X, train_size=train_size, stratify=y)
           train_indices = [list(t) for t in zip(*train_indices)]
           test_indices = [list(t2) for t2 in zip(*test_indices)]
           train_gt[tuple(train_indices)] = gt[tuple(train_indices)]
           test_gt[tuple(test_indices)] = gt[tuple(test_indices)]
    # elif mode == 'fixed':
